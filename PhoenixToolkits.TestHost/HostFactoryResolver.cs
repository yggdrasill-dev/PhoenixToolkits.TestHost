...
internal sealed class HostFactoryResolver
{
	...
	private static readonly TimeSpan _DefaultWaitTimeout = SetupDefaultTimeout();
	...
	private static TimeSpan SetupDefaultTimeout()
		=> (
			...
			uint.TryParse(
				...
				out var timeoutInSeconds)) switch
		{
			(true, _) => Timeout.InfiniteTimeSpan,
			(_, true) => TimeSpan.FromSeconds((int)timeoutInSeconds),
															 ...
															 Action<Exception?>? entrypointCompleted = null)
	{
		...
		return args => new HostingListener(args, assembly.EntryPoint, waitTimeout ?? _DefaultWaitTimeout, stopApplication, configureHostBuilder, entrypointCompleted).CreateHost();
	}
	...
	private static Func<string[], T>? ResolveFactory<T>(Assembly assembly, string name)
	{
		...
		return !IsFactory<T>(factory)
			? null
			: (args => (T)factory!.Invoke(null, new object[] { args })!);
	}
		...
		public object CreateHost()
		{
			...
			var thread = new Thread(() =>
			{
				...
				finally
				{
					...
					entrypointCompleted?.Invoke(exception);
				}
			})
			{
			...
			try
			{
				...
				{
					throw new InvalidOperationException($"Timed out waiting for the entry point to build the IHost after {_DefaultWaitTimeout}. This timeout can be modified using the '{TimeoutEnvironmentKey}' environment variable.");
				}
			}
